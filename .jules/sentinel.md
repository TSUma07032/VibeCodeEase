...
## 2024-08-23 - [Medium] Fix Regular Expression Denial of Service (ReDoS)
**Vulnerability:** The `CodeAnalyzer` used the `RegExp.exec()` method in a `while` loop with a global flag to find simple static string typos ("functon" and "if condtion:").
**Learning:** Using regular expressions to search for static strings, especially in hot paths iterating over many lines of code, introduces overhead and risks Regular Expression Denial of Service (ReDoS) if the regex engine blocks the Extension Host event loop while processing long lines or complex malicious inputs.
**Prevention:** Always prefer `String.prototype.indexOf()` over `RegExp` for static string matching in performance-critical or large-input paths to eliminate ReDoS vulnerabilities and improve overall extension performance.
