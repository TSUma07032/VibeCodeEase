...
## 2024-10-24 - [Medium] Add timeout to LLM API requests
**脆弱性:** Gemini API呼び出し等のLLM APIリクエストにタイムアウト処理が設定されておらず、リクエストが応答を返さない場合に永久にブロックされるリスクがあった。
**学び:** 外部API通信には、タイムアウトが必須である。応答がない場合、無制限にリソースを消費し、アプリケーション全体が機能不全に陥る（リソース枯渇）可能性がある。
**予防策:** ネットワークリクエストを行う際は、常に `request.setTimeout(ミリ秒, コールバック)` を用いて適切なタイムアウト時間を設定し、タイムアウト発生時には安全にリクエストを破棄・エラーハンドリングする。
