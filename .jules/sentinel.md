...
## 2024-09-02 - [Medium] LLM API 呼び出しのタイムアウト追加
**脆弱性:** Gemini API への HTTP リクエストにおいて、`timeout` の設定がなく、サーバーからの応答がない場合にプロセスが永続的にブロックされ、リソース枯渇（DoS）を引き起こすリスクがありました。
**学び:** 外部 API へのネットワークリクエストには常に適切なタイムアウト期間を設けることで、異常時に迅速に失敗させ（Fail Securely）、拡張機能全体がハングアップするのを防ぐ必要があります。
**予防策:** `https.request` のオプションに `timeout: 30000` を追加し、`timeout` イベントで明示的にリクエストを破棄（`destroy`）してエラーを返すようにする。
