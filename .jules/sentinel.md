...
## 2026-08-21 - [High] Prevent Extension Host DoS via Malformed Webview Payloads
**Vulnerability:** The extension processed webview messages without validating the payload structure, assuming `data` was always an object with a `command` string property.
**Learning:** `onDidReceiveMessage` can receive arbitrary data types (e.g., `null`, `string`, `undefined`) from the webview. Attempting to access properties like `data.command` on `null` or primitives causes unhandled exceptions in the Extension Host, leading to potential Denial of Service (DoS) and extension crashes.
**Prevention:** Always implement strict structural payload validation (e.g., verifying `typeof data === 'object'`, `data !== null`, and `typeof data.command === 'string'`) at the entry point of `onDidReceiveMessage` before processing any webview messages.
