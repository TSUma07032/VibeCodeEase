...
## 2024-05-18 - [Medium] Prevent ReDoS in text analysis
**Vulnerability:** The code used `RegExp.exec()` inside a loop for static strings like `functon` and `if condtion:`. This could lead to a Regular Expression Denial of Service (ReDoS) vulnerability if an attacker provided a large input with a pattern that triggered excessive backtracking, potentially blocking the Extension Host event loop.
**Learning:** `RegExp.exec()` can be slow and vulnerable to ReDoS when dealing with large strings or complex patterns. For static string matching, `String.prototype.indexOf()` is much faster and safer.
**Prevention:** Always use `String.prototype.indexOf()` for matching static strings in hot paths, especially when iterating over large text blocks, to avoid ReDoS and performance issues.
