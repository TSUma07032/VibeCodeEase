...
## 2026-08-18 - String Match Optimization in Loops
**学び:** ループ内で毎回 `RegExp.exec()` を生成・実行するのは、静的な文字列検索の場合、極めて非効率である。
**アクション:** 静的な部分文字列の検索には、常に `String.prototype.indexOf()` を使用し、正規表現エンジンのオーバーヘッドを回避する。
