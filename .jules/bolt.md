...
## 2024-09-13 - Case-Insensitive Matching Optimization
**学び:** 大文字小文字を区別しない静的文字列検索において、正規表現 (`/pattern/i.test()`) を使用するよりも、文字列を `.toLowerCase()` で小文字に変換した後にチェーンされた `.includes()` で検索する方が V8 エンジンのオーバーヘッドを避けられ高速である（以前の正規表現を用いた最適化は実際にはパフォーマンスを低下させていた）。
**アクション:** Node.js のホットパス（Diagnosticsのパースなど）では、単純な文字列マッチングに正規表現を用いず `.toLowerCase()` と `.includes()` の組み合わせを優先する。
