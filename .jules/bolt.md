...
## 2026-09-09 - RegExp.test の置き換えによる文字列マッチング最適化
**学び:** V8エンジン環境（Node.js/VS Code）において、単純な部分文字列の検索にインラインの正規表現テスト（`/pattern/i.test(msg)`）を使用すると、正規表現のコンパイルおよび実行時のオーバーヘッドによりパフォーマンスが低下する。
**アクション:** 高頻度で呼び出される文字列判定処理（例えば `categorizeDiagnostic` のような複数パターンの検査）では、事前の小文字化（`toLowerCase()`）と `String.prototype.includes()` を組み合わせることで、イベントループのブロック時間を短縮する最適化パターンを適用する。
