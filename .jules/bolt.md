...
## 2026-09-04 - Cache Optimization for Sequential History Analysis
**学び:** ユーザーの行動履歴やログを配列に蓄積し、そこから連続する特定のアクション（例: 連続承認回数）を算出する処理（`O(N)`）は、配列サイズが大きくなるにつれてパフォーマンスのボトルネックになりやすい。
**アクション:** 履歴を遡及して都度計算するのではなく、アクションが追加されるタイミング（`recordAction`）でカウンタ（`Map` など）を更新し、状態をキャッシュする（`O(1)`）アプローチを優先する。
