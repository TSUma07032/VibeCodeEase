...
## 2024-11-20 - DiagnosticsServiceにおけるカテゴリ判定の最適化
**学び:** 同一の文字列に対して多数の `String.prototype.includes()` を連続して呼び出す場合、それぞれが文字列全体をスキャンするため非効率となる。また事前に `toLowerCase()` を呼び出すコストもかかる。
**アクション:** 複数の部分文字列のいずれかが含まれるかを判定する場合は、コンパイル済みの正規表現(`RegExp.test`)を用いることで、一度のスキャンで判定でき、不要な文字列生成（`toLowerCase`）も省けるため優先する。
