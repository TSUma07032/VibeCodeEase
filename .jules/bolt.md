...
## YYYY-MM-DD - [正規表現の早期リターン最適化]
**学び:** 頻繁に実行されるループ内での正規表現による文字列検索はオーバーヘッドが大きい。V8エンジンでは、単純な静的文字列判定（`String.prototype.endsWith()`など）が非常に最適化されている。
**アクション:** ループ内で正規表現を使用する前に、`endsWith()`や`startsWith()`などの$O(1)$に近い文字列メソッドで早期リターンを行い、不要な正規表現の実行をスキップする。
