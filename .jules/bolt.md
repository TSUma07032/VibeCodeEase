...
## 2024-08-19 - [静的文字列検索ループのパフォーマンス最適化]
**学び:** ホットパス（例: 数千行に対するイテレーション）において、静的な文字列（例: 'functon'）を検索する際、`RegExp.exec()` よりも `String.prototype.indexOf()` の方がはるかに高速です。正規表現エンジンのオーバーヘッドはループ内で顕著になり、拡張機能ホストのイベントループをブロックする原因になり得ます。
**アクション:** 今後、正規表現によるパターンマッチングが不要な静的文字列の検索（特にループ処理内）では、常に `indexOf` を使用し、パフォーマンスの低下とReDoS脆弱性のリスクを最小化します。
