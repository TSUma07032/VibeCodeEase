...
## 2026-08-28 - Fast array lookups with Set in API Clients
**学び:** 外部APIから取得したリストデータ（例：Geminiモデルの一覧）をフィルタリングする際、$O(n)$の `Array.includes` を `filter` 内で使用すると $O(n^2)$ のネストされた配列操作となり、アイテム数が増加した際にパフォーマンスが低下する。
**アクション:** ネストされた配列探索を伴うフィルタリング処理では、事前に探索対象の配列から Set を生成し、$O(n)$ の Map/Set ルックアップ (`Set.has`) に置き換えることで計算量を削減し、初期化処理を高速化する。
