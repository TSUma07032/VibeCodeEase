...
## 2024-11-21 - 正規表現による静的文字列マッチングの回避
**学び:** V8エンジンにおいて、単純な静的文字列の探索を行う場合、複数の条件であっても正規表現（`RegExp.test`）を使用するより、小文字化した文字列に対して `includes` や `indexOf` をチェインさせる方が実行速度が速い。
**アクション:** 単純な部分文字列の検索には、正規表現のオーバーヘッドを避けるため `includes` を優先して使用する。
