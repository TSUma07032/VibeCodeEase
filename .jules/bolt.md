...
## 2026-09-11 - 正規表現を用いた静的文字列マッチングのオーバーヘッド
**学び:** `DiagnosticsService` のような高頻度で実行されるエラー分類処理において、大文字・小文字を無視した単純な静的文字列の探索に `/pattern/i.test()` などの正規表現を使用すると、V8エンジンの正規表現エンジンのオーバーヘッドによりメインスレッドのイベントループをブロックし、パフォーマンス低下の原因となる。
**アクション:** 単純な部分文字列の探索（特に複数の OR 条件がある場合）では、事前に対象文字列を `.toLowerCase()` で小文字化した上で、`.includes()` や `.indexOf()` をチェーンするパターンを優先的に適用する。
