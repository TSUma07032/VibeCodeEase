...
## 2026-09-05 - 頻繁なメソッド呼び出しのメモ化 (Memoization)
**学び:** 言語プロバイダー（Hover, CodeAction）など、高頻度で実行されるループ内で呼ばれるグローバルな状態取得処理（例: `getInterventionLevel`）は、内部で複雑な計算（クランプ処理など）を持たなくとも累積的なパフォーマンス劣化を招く。
**アクション:** 状態が変更される頻度が極めて低い設定値などの取得ロジックには、内部に $O(1)$ のキャッシュ（Map）を設け、状態変更時にのみキャッシュを破棄するメモ化パターンを適用する。
## 2026-09-06 - O(1) Caching for Historical State Traversal
**学び:** 過去の履歴配列を逆順に走査（O(N)）して連続回数などの状態を計算する処理は、配列が肥大化するとパフォーマンスの低下を招く。
**アクション:** 状態の計算結果を保持するO(1)のキャッシュプロパティ（辞書など）を用意し、状態変更時に差分更新するパターンを適用してループ走査を排除する。
...
## 2026-09-08 - HoverProviderにおける不要なオブジェクト生成を早期リターンでスキップ
**学び:** ホバーイベントなどの高頻度で実行されるプロバイダーにおいて、解析結果の配列を走査する際、対象の行より前にある結果に対して `vscode.Range` オブジェクトの生成や包含判定(`contains()`)を無駄に行ってしまうと、メモリ割り当てやCPUサイクルの無駄遣いが発生する。
**アクション:** `HoverProvider` や `CodeActionProvider` などで、行番号によってソートされた結果を走査する場合、対象となる行よりも前（`result.range.end.line < position.line`）の結果に対しては早期に `continue` でスキップし、オブジェクトのインスタンス化を遅延させる最適化パターンを適用する。
