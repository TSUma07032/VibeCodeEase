...
## 2026-09-02 - DiagnosticsServiceにおけるカテゴリ判定処理の正規表現最適化
**学び:** 頻繁に呼び出される `DiagnosticsService.categorizeDiagnostic` のようなホットパスにおいて、複数の `String.prototype.includes()` を `||` で繋げて判定するよりも、`RegExp.prototype.test()` と文字列結合(`|`)を用いたコンパイル済み正規表現の方が高速である。
**アクション:** 複数の静的な部分文字列の包含をチェックする際は、初期化時に1度だけコンパイルされる正規表現を用いた判定（`REGEX.test(msg)`）に置き換える。
