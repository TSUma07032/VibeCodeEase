...
## 2026-08-14 - 正規表現からindexOfへの置き換えによる構文解析の高速化
**学び:** ホットパス（ここでは `analyze` 関数内のループ）において、固定文字列の検索に `RegExp.exec` を使用すると、正規表現のオーバーヘッドによりパフォーマンスが低下する。`String.prototype.indexOf` に置き換えることで、50万行の解析において実行時間を約612msから約216msへと約3倍高速化できることがわかった。
**アクション:** 高速なループ処理内で単純な固定文字列の検索や置換を行う場合は、正規表現を避け、可能な限り `indexOf` を使用する。
