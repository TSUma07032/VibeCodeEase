...
## 2026-08-28 - O(N)の履歴走査をO(1)の状態キャッシュへ最適化
**学び:** ユーザーのアクション履歴など、蓄積され続ける配列に対して逆順走査（O(N)）で特定の条件をカウントする処理は、呼び出し回数や履歴の増加に伴いパフォーマンスを低下させる。
**アクション:** 履歴を都度走査するのではなく、レコード追加時（`recordAction`等）に状態を更新してキャッシュ（O(1)）する設計に変更し、実行時間を一定に保つ。
