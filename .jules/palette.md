...
**学び:** プレーンテキストの代わりに `vscode.MarkdownString` をステータスバーのツールチップ (`statusBarItem.tooltip`) に使用することで、マークダウンフォーマットやテーマアイコンをサポートでき、視認性と表現力が向上する。
**アクション:** UIコンポーネントのツールチップには、可能な限りプレーンテキストの代わりに `vscode.MarkdownString` (`supportThemeIcons = true`) を使用して、開発者体験を向上させる。
