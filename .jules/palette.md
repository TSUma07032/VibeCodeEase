...
## 2024-09-10 - Markdown in StatusBar tooltips
**学び:** StatusBarItem の tooltip にプレーンテキストを指定すると視認性が低く、重要な情報が埋もれてしまう。`vscode.MarkdownString` を使用し、`supportThemeIcons = true` を設定することで、太字やコードブロック、標準アイコンを活用した視覚的に分かりやすいツールチップを提供できる。
**アクション:** StatusBar のツールチップには常に `vscode.MarkdownString` を使用し、適切なマークダウン修飾とアイコンを交えて明確でリッチなテキストを提供する。
