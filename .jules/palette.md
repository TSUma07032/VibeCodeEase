...
## 2024-08-29 - Non-intrusive feedback for automated background tasks
**学び:** 自動修正（保存時サイレント修正など）が行われた際や、設定が自動で更新された時に、`showInformationMessage` で通知を行うと、開発者のタイピングや集中（フロー）を阻害してしまう。このようなバックグラウンド処理の完了通知には、邪魔にならない `setStatusBarMessage`（アイコン付き・タイムアウト付き）を使用する方がUXに優れている。
**アクション:** バックグラウンド処理の完了や、ユーザーの明示的な要求に基づかない設定変更の通知には、ポップアップを使わずにステータスバーを利用する。
## 2026-08-30 - Replace adaptive suggestion popup with status bar message
**学び:** ユーザーがアクション（適応提案の承認など）を行った後に `showInformationMessage` のようなポップアップを表示すると、作業のフローが中断され、さらに「閉じる」という無駄なアクションを強いることになる。
**アクション:** 設定更新や完了通知などの非クリティカルなフィードバックは、必ず `setStatusBarMessage` を使用し、アイコンとタイムアウト（例 `3000` ms）を組み合わせることで、非侵入型で控えめなUXを提供する。
## 2026-09-05 - ユーザーの承認・同意プロセスにおける非侵入型UXの改善
**学び:** 自動修正モード（SILENT）への切り替えのようなユーザーの同意が必要なアクションに対して、作業中に `showInformationMessage` のようなインタラクティブなポップアップを表示すると、開発者のフローを著しく中断させる。
**アクション:** システムがユーザーの行動パターン（連続承認など）から意図を推測できる場合は、ポップアップで確認を求めるのではなく、安全な範囲で自動的に設定をアップグレードし、事後通知として控えめな `setStatusBarMessage` を用いることで、ユーザーに「閉じる」や「はい」を選択させる手間を省き、シームレスな体験を提供する。
## 2024-09-15 - InputBox と QuickPick に Title を追加して出所を明確にする
**学び:** `vscode.window.showInputBox` や `vscode.window.showQuickPick` に `title` プロパティを設定しないと、VS Code UI 上で突然入力や選択を求められたように見え、どの拡張機能からの要求なのかがユーザーに伝わりにくく不安を与える。
**アクション:** ユーザー入力を要求するUI（InputBox, QuickPick）を呼び出す際は、必ず `title` オプションを含め、コンテキスト（例: `vibeCodeEase: ...`）を明確にして、突如現れたように感じさせないUXを提供する。
