...
## 2024-09-12 - Markdown in StatusBar tooltips
**学び:** StatusBarItemのtooltipに `vscode.MarkdownString` を使用することで、アイコンの表示やテキストのフォーマットが可能になり、視認性が向上する。
**アクション:** プレーンな文字列の代わりに `vscode.MarkdownString` を優先して使用し、`supportThemeIcons = true` に設定する。
## 2026-09-12 - Revert AdaptiveEngine prompt to StatusBar notification
**学び:** ユーザーがアクション（適応提案の承認など）を行った後に `showInformationMessage` のようなポップアップを表示すると、作業のフローが中断され、さらに「閉じる」という無駄なアクションを強いることになる。
**アクション:** 設定更新や完了通知などの非クリティカルなフィードバックは、必ず `setStatusBarMessage` を使用し、アイコンとタイムアウト（例 `3000` ms）を組み合わせることで、非侵入型で控えめなUXを提供する。
