...
export function activate(context: vscode.ExtensionContext) {
	...
	const switchModeDisposable = vscode.commands.registerCommand('vibecodeease.switchMode', async () => {
		const items: vscode.QuickPickItem[] = [
			...
			{ label: '$(eye-closed) Off', description: 'Pause assistance' }
		];
		...
		if (selected) {
			...
			if (selected.label.includes('Silent')) {level = 'SILENT';}
			else if (selected.label.includes('Suggest')) {level = 'SUGGESTION';}
			else if (selected.label.includes('Off')) {level = 'IGNORE';}

			statusBar.updateMode(level);
			vscode.window.setStatusBarMessage(`$(check) Mode changed to ${selected.label.split(' ')[1]}`, 3000);
		}
	});
