...
- [ ] AST（抽象構文木）操作・修正案生成ロジック
  ...
  - [x] インデント、命名、ブロック構文の順に修正案生成器を追加する。
    - Jules Memo: 命名規則 (camelCase) の修正案生成ロジックを正規表現でCodeAnalyzerに実装し、VAR_FUNC_MANAGEMENT に対応しました。インデントとブロック構文は未実装のため、Backlogにタスクを追加しました。
