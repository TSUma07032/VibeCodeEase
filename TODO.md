...
- [ ] AST（抽象構文木）操作・修正案生成ロジック
  ...
  - [x] 命名規則（camelCase 補正）の修正案生成器を追加する。
    - Jules Memo: src/core/analyzer.ts に snake_case の変数宣言 (const/let) を camelCase に補正する簡易ロジックを追加し、テストを実装しました。
  - [ ] インデント、ブロック構文の修正案生成器を追加する。
