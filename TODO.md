...
- [x] Webviewから送信されるメッセージアクションの型安全性をさらに高めるリファクタリング
  - Jules Memo: Webviewから受信するメッセージを`ExtensionMessage`として判別可能なUnion型にリファクタリングし、`WebviewMessageHandler.ts`での`as`キャストとアクセスを安全にしました。
- [ ] UI側（React）からのメッセージ送出時にも `ExtensionMessage` に準拠した型安全なユーティリティ関数を導入する。
