...
- [x] 介入判定エンジン (サイレント修正 vs ポップアップ提案)
  ...
  - Jules Memo: `InterventionEngine`クラスを実装し、ユーザーの嗜好値（0.0〜1.0）に基づいて介入レベル（IGNORE, SUGGESTION, SILENT）を決定するロジックを追加しました。
...
- [ ] 今回実装した InterventionEngine を各プロバイダ（Hover, CodeAction 等）と統合し、介入レベルに応じた動作の切り替えを実装する。
