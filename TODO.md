...
- [x] CodeAnalyzerの検出結果をHoverProviderに統合する。
  - Jules Memo: `HoverProvider` と `CodeActionProvider` の両方をリファクタリングし、ハードコードされていたタイポ検出・置換ロジックを `CodeAnalyzer` に委譲しました。`HoverProvider` ではドキュメントバージョン等のキャッシュは利用せず、現在の行のみを解析するようにしてパフォーマンスに配慮しました。
