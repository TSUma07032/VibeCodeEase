...
- [x] 介入判定エンジン (サイレント修正 vs ポップアップ提案)
  ...
  - Jules Memo: `InterventionEngine` クラスを作成し、ユーザーの Pain 嗜好値に基づいて `IGNORE` (< 0.3), `SUGGESTION` (< 0.8), `SILENT` (>= 0.8) のレベルを判定するロジックを実装し、Mochaでテストを追加しました。
...
- [ ] InterventionEngine を CodeAnalyzer または関連機能に統合し、静的なレベル判定を動的判定に置き換える。
